# Requisitos 📋

---

## 🎨 Frontend

### 🤖 Produção

- [Node (^18.16.0)](https://nodejs.org/)
- [Vue (^3.2.47)](https://vuejs.org/)
- [@inkline/inkline (^3.2.2)](https://inkline.io/)

### 🧰 Desenvolvimento

- [Vite (^4.2.0)](https://vitejs.dev/)
- [Sass (^1.62.1)](https://sass-lang.com/)
- [Prettier (^2.8.7)](https://prettier.io/)
- [eslint (^8.38.0)](https://eslint.org/)
- [@typescript-eslint/eslint-plugin (^5.58.0)](https://www.npmjs.com/package/@typescript-eslint/eslint-plugin)
- [@typescript-eslint/parser (^5.58.0)](https://www.npmjs.com/package/@typescript-eslint/parser)
- [@vitejs/plugin-vue (^4.1.0)](https://www.npmjs.com/package/@vitejs/plugin-vue)
- [@vue/eslint-config-prettier (^7.1.0)](https://www.npmjs.com/package/@vue/eslint-config-prettier)
- [@vue/eslint-config-typescript (^11.0.2)](https://www.npmjs.com/package/@vue/eslint-config-typescript)
- [eslint-plugin-prettier (^4.2.1)](https://www.npmjs.com/package/eslint-plugin-prettier)
- [eslint-plugin-vue (^9.10.0)](https://www.npmjs.com/package/eslint-plugin-vue)
- [typescript": (^4.9.5)](https://www.typescriptlang.org/)
- [vue-tsc (^1.2.0)](https://www.npmjs.com/package/vue-tsc)

---

## 🛠️ Backend

### 🤖 Produção

- [Poetry (^1.4.2)](https://python-poetry.org/)
- [Python (^3.11)](https://www.python.org/)
- [FastAPI (^0.95.1)](https://fastapi.tiangolo.com/)
- [uvicorn (^0.21.1)](https://www.uvicorn.org/)
- [SQLAlchemy (^2.0.13)](https://www.sqlalchemy.org/)
- [pydantic (^1.10.7)](https://pydantic-docs.helpmanual.io/)
- [psycopg2-binary (^2.9.6)](https://www.psycopg.org/)
- [python-dotenv (^1.0.0)](https://pypi.org/project/python-dotenv/)
- [python-jose[cryptography] (^3.3.0)](https://python-jose.readthedocs.io/en/latest/)
- [passlib[bcrypt] (^1.7.4)](https://passlib.readthedocs.io/en/stable/)
- [python-multipart (^0.0.6)](https://pypi.org/project/python-multipart/)


### 🧰 Desenvolvimento

- [Pytest (^7.3.1)](https://docs.pytest.org/)
- [Pytest-cov (^4.0.0)](https://pytest-cov.readthedocs.io/en/latest/)
- [Blue (^0.9.1)](https://blue.readthedocs.io/en/latest/)
- [iSort (^5.12.0)](https://pycqa.github.io/isort/)
- [TaskiPy (^1.10.4)](https://github.com/taskipy/taskipy)
- [httpx (^0.24.0)](https://www.python-httpx.org/)

---

## 🗃️ Banco de Dados

- [Docker (^23.0.3)](https://www.docker.com/)
- [PostgreSQL (^15.2)](https://www.postgresql.org/)
- [PgAdmin4 (^7.0)](https://www.pgadmin.org/)

---

## 📚 Documentação

- [Python (^3.11)](https://www.python.org/)
- [Mkdocs-material (^9.1.5)](https://squidfunk.github.io/mkdocs-material/)
- [Mkdcostrings (^0.20.0)](https://mkdocstrings.github.io/)
- [Mkdocstrings-python (^0.9.0)](https://mkdocstrings.github.io/python/)
- [Termynal (^0.2.1)](https://daxartio.github.io/termynal/)
